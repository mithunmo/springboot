...
import com.f9g4.dao.TrCompanyAvailCreditDAO;
import com.f9g4.dao.TrCompanyCreditDAO;
import com.f9g4.dao.TrCompanyCreditTransactionDetailDAO;
import com.f9g4.dao.TrCompanyCreditTransactionHeaderDAO;
...
public class MembershipServiceImpl implements IMembershipService {
	...
	@Autowired
	private TrCompanyCreditDAO companyCreditDAO;
	
	@Autowired
	private TrCompanyAvailCreditDAO availCreditDAO;
	
	@Autowired 
	private TrCompanyCreditTransactionHeaderDAO companyCreditTxnHeaderDAO;

	@Autowired 
	private TrCompanyCreditTransactionDetailDAO companyCreditTxnDetailDAO;
...
//										, LookupTypeValueEnum.MEMEBERSHIP_TYPE_STANDARD.getTypeCode());
	...
	public AddCreditResponse addCredit(AddCreditRequest request) {
		...
		try {
			...
			TrCompanyCredit companyCredit = new TrCompanyCredit();
			companyCredit.setCreatedBy(request.getCompanyCredit().getCreatedby());
			companyCredit.setLastUpatedBy(request.getCompanyCredit().getLastupdateby());
			companyCredit.setAdUser(adUsersDAO.findAdUsersByUserId(request.getCompanyCredit().getUserId()));
			companyCredit.setTotalCreditAmount(request.getCompanyCredit().getTotalCreditAmount());
			companyCredit.setTotalCredits(request.getCompanyCredit().getTotalCredits());
			companyCredit.setCreditVal(request.getCompanyCredit().getCreditVal());
			companyCredit.setCreationDate(Calendar.getInstance());
			companyCredit.setLastUpdateDate(Calendar.getInstance());		
	...
	public RedeemUserCreditResponse redeemCredit(int userId, int credits) {
		RedeemUserCreditResponse response = new RedeemUserCreditResponse();
		Status status=new Status();
		response.setStatus(status);
		try {
			Long total = availCreditDAO.findTotalAvailCreditByUserId(userId, -1, -1);
			
			if (total >= credits ){
				
				int creditUsed = credits;
				
				TrCompanyCreditTransactionHeader companyCreditTxnHeader = new TrCompanyCreditTransactionHeader();
				companyCreditTxnHeader.setCreatedBy(userId);
				companyCreditTxnHeader.setTotalCredits(credits);
				companyCreditTxnHeader.setCreationDate(Calendar.getInstance());
				companyCreditTxnHeader = companyCreditTxnHeaderDAO.merge(companyCreditTxnHeader);													
				Set<TrCompanyAvailCredit> creditList  =	 availCreditDAO.findAvailCreditByUserId(userId, -1, -1);
				for ( TrCompanyAvailCredit  item : creditList ){
					
					if ( creditUsed <= item.getTotal() && item.getTotal() !=0   ) {						
						TrCompanyCreditTransactionDetail companyCreditTxnDetail = new TrCompanyCreditTransactionDetail();
						companyCreditTxnDetail.setTxnHeaderId(companyCreditTxnHeader);
						companyCreditTxnDetail.setNumCredits(creditUsed);
						companyCreditTxnDetail.setCompanyCreditId(item.getCompanyCreditId());
						companyCreditTxnDetailDAO.merge(companyCreditTxnDetail);
						break;						
					} else if ( creditUsed > item.getTotal() && item.getTotal() != 0  ) {								
						creditUsed = creditUsed - item.getTotal();
						TrCompanyCreditTransactionDetail companyCreditTxnDetail = new TrCompanyCreditTransactionDetail();
						companyCreditTxnDetail.setTxnHeaderId(companyCreditTxnHeader);
						companyCreditTxnDetail.setNumCredits(item.getTotal());
						companyCreditTxnDetail.setCompanyCreditId(item.getCompanyCreditId());
						companyCreditTxnDetailDAO.merge(companyCreditTxnDetail);												
					}					
				}								
				...
				status.setStatuscode(StatusCodesEnum.STATUS_ERR_UNKNOWN.getCode());
				status.setDescription("Insufficient Credits");				
			}							
		} catch ( Exception e){
			e.printStackTrace();
			status.setStatuscode(StatusCodesEnum.STATUS_ERR_UNKNOWN.getCode());
