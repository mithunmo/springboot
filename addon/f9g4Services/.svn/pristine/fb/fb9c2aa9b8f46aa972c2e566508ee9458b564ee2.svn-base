...
public class TrPortfolioContestDAOImpl extends AbstractJpaDao<TrPortfolioContest>
	...
	public TrPortfolioContest findTrPortfolioByContestIdAndPortfolioId(
			Integer portfolioId, Integer contestId, int startResult, int maxRows) {
		try {
			Query query = createNamedQuery("findTrPortfolioByContestIdAndPortfolioId", startResult, maxRows, portfolioId, contestId);
				return (TrPortfolioContest)query.getResultList().get(0);
		} catch (NoResultException nre) {
			return null;
		}
