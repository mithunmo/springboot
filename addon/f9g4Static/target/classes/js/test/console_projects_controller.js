...
app.controller('editProjectBoardController', ['$scope','$http','$filter','$rootScope','$routeParams','FormValidator','dialogs','$location','$log','BoardService','mode', function($scope,$http,$filter,$rootScope,$routeParams,validator,dialogs,$location,$log,boardService,mode) {
	...
	if(mode=='edit') {
		...
		$scope.form.contestId=$routeParams.contestId;
	}
	//
	...
	$scope.clear = function(clearAll) {
		...
		if(clearAll) {
			...
			$scope.form={
				...
				contestId: $scope.form.contestId
			};
		}
		else {
			$scope.form={
	...
	function boardHandler(publishToMp) {
		...
		var confirmDialogMsg='This board will NOT be published to the Project. Would you like to continue? If you would like your board to be visible in the Project, you must select "Save & Publish."'
			...
			,changeViewDialogMsg='Do you want to upload another submission?';
		if(publishToMp==1) {
					...
					if(data.hasError==false) {
						...
						notifyDialog.result.then(function(){
							...
							confirmDialog.result.then(function(btn){
								...
								//NO, then change view
								$location.path(ajaxPaths['viewProjectsPortfolioPath']+'/'+data.board.portfolioId+'/'+$scope.form.contestId);
							});
						});
					}
					else {
...
app.controller('viewProjectsBriefController', ['$scope', '$http', '$filter','$rootScope','$routeParams','$log','BoardService', function($scope,$http,$filter,$rootScope,$routeParams,$log,boardService) {
	$log.log('designer view creative brief  controller');
	...
	$scope.injectData = function(data) {
		$scope.contests=data.targetContest;
		$scope.hasError=data.hasError;
	}
	
	$http({method: 'GET', url: ajaxPaths['viewProjectsCreativeViewDataPath'], params: { portfolioId:$routeParams.portfolioId, contestId:$routeParams.contestId }})
	.success(function(data, status, headers, config) {
		$scope.isLoading=false;
		$log.log(data);
		$scope.injectData(data);
...
app.controller('viewDesignerProjectsController', ['$scope', '$http', '$filter','$rootScope','BoardService','$location', function($scope,$http,$filter,$rootScope,boardService,$location) {
	...
	$scope.boardsView= function(portfolioId, contestId) {
		$location.path(ajaxPaths['viewProjectsPortfolioPath']+'/'+portfolioId+'/'+contestId);
	};
	$scope.cbdView= function(portfolioId, contestId) {
		window.location.href = ajaxPaths['viewCBDPath']+"?portfolioId="+portfolioId+"&contestId="+contestId;
	};	
	$scope.creativeBriefView = function(portfolioId, contestId) {
		$location.path(ajaxPaths['viewProjectsCreativeViewPath']+'/'+portfolioId+'/'+contestId);
	};
