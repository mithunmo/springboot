...
$(document).ready(function()
{
	...
	$("#tabs").tabs(
	{
		...
		select:function(e,ui)
		{
			...
			else
			{
				...
				var tabSelectHandler = {
					...
					"#tab_designer_home_edit_mode":function() {
						$("#tab_designer_home_edit_mode").html(layoutTemplate.basicConsoleFrame); //show loading message
					},
		...
		show: function(e,ui)
		{
			...
			var tab=ui.tab.hash;
			var tabShowHandler= {
				"#tab_init_raty_activity":function (){
...
/*			var r=confirm("It will NOT publish your board to MarketPlace. Continue?");
	...
	$(document).on("click","#cancel_create_board",function(e)
	{
		...
		if(globalVars.isChangedWithoutSubmit)
		{
			...
			/*$("body").bind("changedWithoutSaveOnCreateBoard",function(changeEvent)
			{	
				if(changeEvent.trigger=="ok")
				{
					//set the flag false;
				...
				$("body").off("changedWithoutSaveOnCreateBoard");
			});*/
		}
		else
			window.history.back();
	});
	$(document).on("click","#portfolioId",function(e)
	{
	...
	$(document).on("change",'#expDays input[name="expirationDays"]',function()
	{
		...
		$.ajax({
			...
			success: function(result)
			{
				$('#current_exp_days span').html(result.expirationDate+" (Based on today)");
			}
		});
	});
	...
	//manage board tab========================================================================================
	$(document).on("click","#show_all_portfolios a",function(e)
	{
	...
	$(document).on("mouseenter","#manage_board .designerBoardLink",function(e) //show effect when mouse move in target portfolio
	{
		//$(this).children("div").eq(0).addClass("shadow"); //add shadow
		$(this).parent().addClass("shadow"); //add shadow
	});
	$(document).on("mouseleave","#manage_board .designerBoardLink",function(e) //show effect when mouse move out target portfolio
	{
...
function editBoard(boardId)
{
	...
	$.ajax({
		...
		success: function (result) {
			...
			{
				buildPopupMsg({
					"content":"The board is being purchased. You cannot edit this board.",
					"showOk":true,
					"onOk":function(){
						closePopupMsg();
					}
