...
public class DomainToFormConverter {
	...
	public ContestBean convertDomainToForm(TrPortfolioContestDomain source, HashMap<Integer, StSeasonYearDomain> seasonYearHM, Integer timezoneOffset, List<AdLookupValuesDomain> boardDimension, List<AdLookupValuesDomain> boardFormat )
	{
		ContestBean bean = new ContestBean();
		bean.setOriginalData(source);
		//normalize data
		...
		bean.setEndDate(endDate); //convert to local time
		bean.setSeasonYear(seasonYearHM.get(source.getContestPortfolioDomain().getSeasonYearId()));
		bean.setBoardDimension(boardDimension);
		bean.setBoardFormat(boardFormat);
		return bean;
	}
