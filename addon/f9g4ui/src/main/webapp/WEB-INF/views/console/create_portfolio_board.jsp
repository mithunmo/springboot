    ...
    <form:form modelAttribute="createBoardForm" id="form_create_board" method="POST">
    	<div class="content-function">
        	<div class="oldPortfolio">
                ...
                <input type="hidden" name="portfolioId" id="portfolioId" value="${portfolioList[0].portfolioId}" />
            </div>
    	</div>
        <div class="upload_board">
            ...
            <div class="button" id="upload_ai" data-mode="p">Upload</div>
            ...
            <!-- keywords fields -->
           	<div class="keywords-session">
           		<table>
	            	...
	            	<tr>
	           			<td class="keyword-label">OTHER TAGS<%-- KEY WORDS (Others) --%></td>
        ...
        <div class="detail">
        	<div class="leftArea">
            	<span id="label_board_name" class="font-header cap-bold">Title</span>
                <br />
                ...
                <table id="saleTypeTable">
                  ...
                  <c:forEach var="item" items="${saleTypeList}" varStatus="rowCounter">
                	<tr  class="grayBackground">
                  		<td>
                  			<c:choose>
                  				<c:when test="${empty boardDetail && item.lookupValueId==442}">
                  		...
                  		<c:choose>
                  			<c:when test="${empty boardDetail && item.lookupValueId==442}">
                  				<td id="saletype_${rowCounter.count}"><input name="saletype_${rowCounter.count}_cost" type="text" id="form_saletype_${rowCounter.count}_cost" class="cost" value="0000.00"/></td>
                  			</c:when>
                  			<c:when test="${boardDetail.saleTypeId == item.lookupValueId}">
                  				<td id="saletype_${rowCounter.count}"><input name="saletype_${rowCounter.count}_cost" type="text" id="form_saletype_${rowCounter.count}_cost" class="cost" value="${boardDetail.boardPrice}"/></td>
                  			</c:when>
                  			<c:otherwise>
                  				<td id="saletype_${rowCounter.count}">0000.00</td>
                  			</c:otherwise>
                  		</c:choose>     		
...
that it is not displayed on any other websites and that it is my original work.</label></td></tr>
            ...
            <div class="buttons">
            	<table style="width:100%;">
            		<tr>
            			<td class="td">
            				<div class="button" id="save_create_pboard">Save</div>
            			</td>
            			<td class="td">
            				<div class="button" id="publish_create_pboard">Save &amp; Publish</div>
            			</td>
            			<td class="td">
            			...
            			<td class="td">
            				<c:choose>
            					<c:when test="${hasWithPortfolio}">
            						<div class="button orange" id="back_to_pboards" data-portfolio="${portfolioId}">Cancel</div>
            					</c:when>
            					<c:otherwise>
            						<div class="button orange" id="back_to_portfolio">Cancel</div>
            					</c:otherwise>
            				</c:choose>
            			</td>
            		</tr>
            		<tr>
            			<td>
            				<span class="only"><spring:message code="page.manage.save.hint"></spring:message></span>
            			</td>
            			<td>
            				<span class="all"><spring:message code="page.manage.publish.hint"></spring:message></span>
            			</td>
            			<td></td>
            			<td></td>
            		</tr>
