...
<!-- Content -->
<div class="content content-wrapper" ng-controller="adminSearch">
	<div class="content-holder">
		<div class="title">ADMINISTRATOR</div>
		<div class="date-range">
			<div class="col-md-6">
				<p class="input-group" ng-controller="datePickerController">
	              <input type="text" class="form-control" datepicker-popup="{{format}}" ng-model="$parent.dt_start" is-open="opened" max-date="$parent.dt_end" datepicker-options="dateOptions" date-disabled="disabled(date, mode)" ng-required="false" close-text="Close" placeholder="Date From"/>
            ...
            <div class="col-md-6">
				<p class="input-group" ng-controller="datePickerController">
	              <input type="text" class="form-control" datepicker-popup="{{format}}" ng-model="$parent.dt_end" is-open="opened" min-date="$parent.dt_start" datepicker-options="dateOptions" date-disabled="disabled(date, mode)" ng-required="false" close-text="Close" placeholder="Date To"/>
	              <span class="input-group-btn">
	                <button type="button" class="btn btn-default" ng-click="open($event)"><i class="glyphicon glyphicon-calendar"></i></button>
	              </span>
	            </p>
		...
		<div class="tabs">
			<div class="header">
				<a class="item selected" href="${pageContext.request.contextPath}<%= ViewPaths.ADMIN_SEARCH_CRITERIA%>" target="_self" ng-click="show()" data-target="1">Refine Search</a>
				<a class="item" href="${pageContext.request.contextPath}<%= ViewPaths.ADMIN_SEARCH_VIEWER%>#portfolioPages" ng-click="show()" data-target="2">Portfolio Pages</a>
				<a class="item" href="${pageContext.request.contextPath}<%= ViewPaths.ADMIN_SEARCH_VIEWER%>#marketplaceSubmissions" ng-click="show()" data-target="3">Marketplace</a>
				<a class="item" href="${pageContext.request.contextPath}<%= ViewPaths.ADMIN_SEARCH_VIEWER%>#examples" ng-click="show()" data-target="3">Examples</a>
			</div>
			<div class="function">
			</div>
			<div class="body">
					...
					<div class="groups">
						<div class="group-holder">
							<div class="group-content">
								<div class="cpanel">
									<div class="cpanel-title">ADMIN RATING:&nbsp;{{adminRatingSlider.min}} - {{adminRatingSlider.max}}</div>
									<div class="cpanel-content">
										<rzslider
										    rz-slider-floor="adminRatingSlider.floor"
										    rz-slider-ceil="adminRatingSlider.ceil"
										    rz-slider-model="adminRatingSlider.min"
										    rz-slider-high="adminRatingSlider.max" rz-slider-step="{{adminRatingSlider.step}}"></rzslider>
									</div>
								</div>
							</div>
						</div>
